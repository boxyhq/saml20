...
saml11.parse = function parse(assertion) {
  ...
  if (attributes) {
    ...
    attributes.forEach(function attributesForEach(attribute) {
      var attributeName =
        attribute['@'].AttributeNamespace + '/' + attribute['@'].AttributeName;

      if (attribute.AttributeValue) {
        claims[attributeName] =
          attribute.AttributeValue._ || attribute.AttributeValue;
      }
    });
  }

  var nameIdentifier = _.get(
    assertion,
    'AttributeStament.Subject.NameIdentifier'
  );

  if (nameIdentifier) {
    claims[
      'http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier'
    ] = nameIdentifier._ || nameIdentifier;
  }
